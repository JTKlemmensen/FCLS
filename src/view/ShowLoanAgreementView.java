...
public class ShowLoanAgreementView implements View
{
	...
	private GridPane createCustomerInfoGrid() {
		...
		customerNameLabel.textProperty()
				.bind(Bindings.concat(customer.firstNameProperty(), " ", customer.lastNameProperty()));
	...
	private GridPane createLoanInfoGrid() {
		...
		Label yearlyPaymentPercentageLabel = new Label(theController.getAPR());

		Label montlyPaymentHeader = new Label("Månedlig Ydelse");
		montlyPaymentHeader.setId("header_label");

		Label montlyPaymentLabel = new Label(theController.getMonthlyPayment());
	...
	private Node createLoanPaymentOverview() {
		...
		ObservableList<Payment> payments = (ObservableList<Payment>) theController.getPayments();
